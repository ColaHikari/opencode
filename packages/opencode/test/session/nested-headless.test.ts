...
describe("session.nested-headless", () => {
  ...
  // ===========================================================================
  it.live(
    "drives a depth-3 spawn chain through the real loop: tree nests, results bubble up, level-4 (=maxDepth) loses task",
    () =>
      ...
      withHeadless(
        ...
        ({ sdk, llm }) =>
          Effect.gen(function* () {
            ...
            // (=maxDepth) does NOT, but is still a full work level (read
            // survives).
